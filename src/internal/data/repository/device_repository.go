...
func (receiver *DeviceRepository) GetDeviceListByUserID(userID string) ([]entity.SDevice, error) {
	var devices []entity.SDevice
	err := receiver.DBConn.Exec("SELECT * FROM s_device WHERE id IN (SELECT device_id FROM s_user_devices WHERE user_id = ?)", userID).Scan(&devices).Error
	return devices, err
}

func (receiver *DeviceRepository) GetDeviceListByOrgID(orgID string) ([]entity.SDevice, error) {
	var devices []entity.SDevice
	err := receiver.DBConn.Exec("SELECT * FROM s_device WHERE id IN (SELECT device_id FROM s_org_devices WHERE organization_id = ?)", orgID).Scan(&devices).Error
	return devices, err
}
