...
func (receiver *SyncSubmissionUseCase) Execute() {
	...
	if !isWorkingTime {
		...
		// s, err := receiver.SubmissionRepository.FindFirstPendingSync()
		...
		// s, err := receiver.SubmissionRepository.FindFirstPendingPrioritizedSync()
	...
	// 	receiver.syncSignUp(submission)
	// case value.SubmissionTypeSignUpWriteToMemoryForm:
	// 	receiver.updateMemoryForm(submission)
	// default:
	// 	break
	// }
	inProcessSubmissionId = 0
...
func (receiver *SyncSubmissionUseCase) getFirstOutputRows(spreadSheetID string, sheetName string) ([]string, error) {
	raw, err := receiver.UserSpreadsheetReader.GetFirstRow(sheet.ReadSpecificRangeParams{
...
func (receiver *SyncSubmissionUseCase) get2FirstOutputRows(spreadSheetID string, sheetName string) ([]uniqueQuestionPair, error) {
	raw, err := receiver.UserSpreadsheetReader.GetFirstRow(sheet.ReadSpecificRangeParams{
		SpreadsheetId: spreadSheetID,
...
func (receiver *SyncSubmissionUseCase) checkExistingSignUpForm(question string, existingQuestions []string) bool {
...
func (receiver *SyncSubmissionUseCase) checkExisting(questionInForm model.FormQuestionItem, questionsOnSheet []uniqueQuestionPair) bool {
...
func (receiver *SyncSubmissionUseCase) findSignUpFormQuestion(question string, questions []model.FormQuestionItem) *model.FormQuestionItem {
...
func (receiver *SyncSubmissionUseCase) findFormQuestion(questionPair uniqueQuestionPair, questions []model.FormQuestionItem) *model.FormQuestionItem {
	for _, formQuestion := range questions {
...
func (receiver *SyncSubmissionUseCase) findAnswer(question *model.FormQuestionItem, answers []entity.SubmissionDataItem) *string {
...
func (receiver *SyncSubmissionUseCase) sync(submission entity.SSubmission) {
	...
	// formQuestions, err := receiver.QuestionRepository.GetQuestionsByFormId(submission.FormId)
	// if err != nil {
	...
	// firstRows, err := receiver.get2FirstOutputRows(submission.SpreadsheetId, submission.SheetName)
	// if err != nil {
	...
	// 	// )
	// 	// _ = receiver.SubmissionRepository.MarkStatusAttempted(submission.ID)
	// 	return
	// }
	...
	// 	if receiver.checkExisting(formQuestion, firstRows) == false {
	...
	// 	if index > 11 {
	// 		if q := receiver.findFormQuestion(question, formQuestions); q != nil {
	// 			if ans := receiver.findAnswer(q, submissionData.Items); ans != nil {
	...
	// }
	// allSubmissions, err := receiver.UserSpreadsheetReader.Get(getAllSubmissionRowsParams)
	// if err != nil {
	...
	err = receiver.sendMessage(submission)
	if err != nil {
		...
		// 	fmt.Sprintf("Spreadsheet: %s%s", "https://docs.google.com/spreadsheets/d/", submission.SpreadsheetId),
		// )
		// err = receiver.SubmissionRepository.MarkStatusSucceeded(submission.ID)
		return
	}

	// writtenRanges, err := receiver.UserSpreadsheetWriter.WriteRangesAsUserEntered(params, submission.SpreadsheetId)
	...
	defer func() {
		receiver.updateRecentSubmissionToFormSpreadsheet(submission, submissionData)
	}()

	defer func() {
		receiver.saveCountingDataIfNeeded(submission, submissionData)
	}()
...
func (receiver *SyncSubmissionUseCase) sendMessage(submission entity.SSubmission) error {
	...
	userDevices, err := receiver.UserEntityRepository.GetUserDeviceById(submission.DeviceId)
	if err != nil {
	...
	for _, ud := range *userDevices {
		...
		for _, item := range submissionData.Items {
			...
			r, err := receiver.GetSettingMessageUseCase.Execute(*user)
			if err != nil {
				monitor.SendMessageViaTelegram(
			...
			err = receiver.SendEmailUseCase.SendMessage(subject, bccList, body)
			if err != nil {
				monitor.SendMessageViaTelegram(
...
func (receiver *SyncSubmissionUseCase) syncSignUp(submission entity.SSubmission) {
	// monitor.SendMessageViaTelegram(
	// 	fmt.Sprintf("[INFO] Sync submission: %d", submission.ID),
	// 	fmt.Sprintf("Target Form Note %s - Form Name %s", submission.FormNote, submission.FormName),
	// 	fmt.Sprintf("Submited by %s at %s", submission.DeviceName, submission.CreatedAt.Format("2006-01-02 15:04:05")),
	...
	// firstRows, err := receiver.getFirstOutputRows(submission.SpreadsheetId, submission.SheetName)
	// if err != nil {
	...
	// if receiver.checkExistingSignUpForm(question.Question, firstRows) == false {
	...
	// for index, question := range allQuestions {
	// 	if index > 11 {
	// 		if q := receiver.findSignUpFormQuestion(question, formQuestions); q != nil {
	// 			answerRow = append(answerRow, []interface{}{receiver.findAnswer(q, submissionData.Items)})
	// 			//formSubmission.Submit = append(formSubmission.Submit, value.Submit{
	// 			//	Question: q.Question,
	// 			//	Answer:   receiver.findAnswer(q.QuestionId, submissionData.Items),
	// 			//})
	// 		} else {
	// 			answerRow = append(answerRow, []interface{}{""})
	// 		}
...
func (receiver *SyncSubmissionUseCase) checkExisting2(question string, existingQuestions []string) bool {
	for _, existingQuestion := range existingQuestions {
		if question == existingQuestion {
			return true
		}
...
func (receiver *SyncSubmissionUseCase) updateRecentSubmissionToFormSpreadsheet(submission entity.SSubmission, submissionData entity.SubmissionData) {
	form, err := receiver.FormRepository.GetFormById(submission.FormId)
	if err != nil {
	...
	formQuestionKeys, err := receiver.UserSpreadsheetReader.Get(sheet.ReadSpecificRangeParams{
	...
	for i := 0; i < len(formQuestionKeys); i++ {
		if len(formQuestionKeys[i]) != 0 && formQuestionKeys[i][0].(string) != "" {
			...
			for j := 0; j < len(submissionData.Items); j++ {
				if strings.EqualFold(submissionData.Items[j].QuestionId, (form.Note + "_" + form.SpreadsheetId + "_" + formQuestionKeys[i][0].(string))) {
	...
	_, err = receiver.UserSpreadsheetWriter.UpdateRange(params, form.SpreadsheetId)
...
func (receiver *SyncSubmissionUseCase) findFormQuestion2(question string, questions []model.FormQuestionItem) *model.FormQuestionItem {
	for _, formQuestion := range questions {
		if formQuestion.Question == question {
			return &formQuestion
		}
...
func (receiver *SyncSubmissionUseCase) updateMemoryForm(submission entity.SSubmission) {
	// monitor.SendMessageViaTelegram(
	// 	fmt.Sprintf("[INFO] Sync submission: %d", submission.ID),
	// 	fmt.Sprintf("Target Form Note %s - Form Name %s", submission.FormNote, submission.FormName),
	// 	fmt.Sprintf("Submited by %s at %s", submission.DeviceName, submission.CreatedAt.Format("2006-01-02 15:04:05")),
	...
	// newQuestions := make([]interface{}, 0)
	// for _, question := range formQuestions {
	// 	if receiver.checkExisting2(question.Question, firstRows) == false {
	// 		newQuestions = append(newQuestions, question.Question)
	// 	}
	// }
	...
	// for index, question := range allQuestions {
	// 	if index > 11 {
	// 		if q := receiver.findFormQuestion2(question, formQuestions); q != nil {
	// 			answerRow = append(answerRow, []interface{}{receiver.findAnswer(q, submissionData.Items)})
	// 			//formSubmission.Submit = append(formSubmission.Submit, value.Submit{
	// 			//	Question: q.Question,
	// 			//	Answer:   receiver.findAnswer(q.QuestionId, submissionData.Items),
	...
	// err = receiver.MarkStatusSucceeded(submission.ID)
	// if err != nil {
	// 	log.Error("Error when mark status succeeded: ", err)
	// }
}
