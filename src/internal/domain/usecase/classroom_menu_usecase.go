...
type ClassroomMenuUseCase struct {
	ClassroomMenuRepository *repository.ClassroomMenuRepository
	ComponentRepository     *repository.ComponentRepository
}

func (uc *ClassroomMenuUseCase) GetClassroomMenu4GW(classroomID string) (response.GetClassroomMenuResponse, error) {

	// B1: Lấy các bản ghi teacher_menu
	departmentMenus, err := uc.ClassroomMenuRepository.GetByClassroomID(classroomID)
	if err != nil {
		return response.GetClassroomMenuResponse{}, err
	}
	...
	return response.GetClassroomMenuResponse{
		Components: componentResponses,
	}, nil
}

func (uc *ClassroomMenuUseCase) GetDepartmentMenu4App(departmentID string) (response.GetClassroomMenuResponse, error) {

	departmentMenus, err := uc.ClassroomMenuRepository.GetByClassroomID(departmentID)
	if err != nil {
		return response.GetClassroomMenuResponse{}, err
	}
