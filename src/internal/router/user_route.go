...
func setupUserRoutes(engine *gin.Engine, dbConn *gorm.DB, config config.AppConfig, consulClient *api.Client, cacheClientRedis *cache.RedisCache) {
	...
	studentApplication := engine.Group("/v1/user/student/application")
	{
		...
		studentApplication.GET("/:id/language-config", secureMiddleware.Secured(), userEntityController.GetStudentLanguageConfig4App)
	}
	...
	userMenu := engine.Group("v1/user-menu", secureMiddleware.Secured())
	{
		...
		userMenu.GET("/student/organization/:organization_id", menuController.GetStudentMenuOrganization4App)
	...
	{
		org.GET("/:organization_id", orgController.GetOrganizationByID4App)
