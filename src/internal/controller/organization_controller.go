...
func (receiver OrganizationController) GetOrganizationByID4Web(context *gin.Context) {
	organizationID := context.Param("organization_id")
	if organizationID == "" {
		context.JSON(
			http.StatusBadRequest, response.FailedResponse{
	...
	organization, err := receiver.GetOrganizationUseCase.GetOrganizationByID4Web(context, organizationID)
	if err != nil {
		context.JSON(http.StatusInternalServerError, response.FailedResponse{
			Error: err.Error(),
			Code:  http.StatusInternalServerError,
