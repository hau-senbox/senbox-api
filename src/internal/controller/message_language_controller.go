...
func (ctrl *MessageLanguageController) DeleteMessageLanguage4GW(ctx *gin.Context) {
	typeStr := ctx.Query("type")
	typeID := ctx.Query("type_id")
	...
	err := ctrl.messageLanguageUseCase.DeleteMessageLanguage4GW(typeStr, typeID)
	if err != nil {
		ctx.JSON(
			http.StatusInternalServerError, response.FailedResponse{
				Code:    http.StatusInternalServerError,
				Error:   err.Error(),
				Message: "Failed to delete message languages",
			},
		)
		return
	}
	...
	ctx.JSON(
		http.StatusOK, response.SucceedResponse{
			Code:    http.StatusOK,
			Message: "Delete message languages success",
			Data:    nil,
		},
	)
}
